...
class Remote(object):
    def __init__(self, hostname, ssh_username, ssh_password, ssh_identity_file, ssh_lib, host_variables):
        self.remote = None
        if ssh_lib == 'paramiko' or ssh_lib == 'ssh2':
            # Removed the ssh2 implementation because it required dependencies
            # that can be hard to install for no real improvement.
            self.remote = RemoteParamiko(hostname, ssh_username, ssh_password, ssh_identity_file, host_variables)
        else:
            raise BadArgument("ssh-lib should be 'paramiko' but we got '%s' instead." % (ssh_lib,))
